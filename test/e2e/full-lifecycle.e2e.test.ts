...
describe("Full Treasury Lifecycle", () => {
  ...
  describe("Step 2: Token Deployment", () => {
    it("deploys ClawcoinToken with 80/20 split to Safe/Vesting", async () => {
      ...
      let TokenVestingArtifact: { abi: unknown[]; bytecode: string };
      let ClawcoinTokenArtifact: { abi: unknown[]; bytecode: string };
      try {
  ...
  describe("Step 9: Audit log integrity", () => {
    ...
    it("audit log entries are queryable", async () => {
      const spendingEntries = await auditLog.query({ category: "spending" });
      expect(spendingEntries.length).toBeGreaterThanOrEqual(1);

      const emergencyEntries = await auditLog.query({ category: "emergency" });
      expect(emergencyEntries.length).toBeGreaterThanOrEqual(2);

      const proposalEntries = await auditLog.query({ category: "proposal" });
